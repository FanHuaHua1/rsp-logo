...
class DecisionTree(
        ...
        tail: Double = 0.05,
        var maxDepth: Int = 10,
        var nodeSize: Int = 10) extends LogoClassification with LogoClassifier[smileDT]{
  ...
  override def run(saveModel:Boolean, saveModelPath:String, doEvaluate: Boolean): RDD[(smileDT, Double)]  = {
    val modelRdd: RDD[(smileDT, Double, Double)] = etl(trainRdd).map(
      sample => {
    ...
    val valuedModels: RDD[(smileDT, Double)] = GO(modelRdd)
    if (saveModel) {
      val saveModelPair: RDD[Pair[smileDT, Double]] = valuedModels.map(f => new Pair(f._1, f._2))
