...
class FrequentItemsetGenerator(object):
    ...
    def generate_from_batch(self, batches: list, samples: int):
        data = list(self.gen_empty_list(samples))
        for batch in batches:
...
def test_generate():
    fig = FrequentItemsetGenerator(range(100))
    df, prop = fig.generate_df()
    print(df)
    print(prop)


def test_batch(filename: str="batch.parquet"):
    fig = FrequentItemsetGenerator(range(100))
    itemset_sizes = [7] + 2*[5] + 2*[4] + 4*[3]
    batches = fig.proportion_batches(itemset_sizes)
    print(batches[0])
    df = fig.batch2df(batches)
    # print(df)
    fig.write_batch_parquet(filename, df)
    bdf = fig.read_batch_parquet(filename)
    # print(bdf)
    bbatches = fig.df2batch(bdf)
    print(bbatches[0])


def test_generate_parquet(filepath: str, batchfile: str, numbers: int):
    import pyarrow as pa
    from pyarrow import parquet as pq
    import os
    fig = FrequentItemsetGenerator(range(800))
    if not os.path.exists(batchfile):
        itemset_sizes = [7] + 2*[5] + 2*[4] + 4*[3]
        batches = fig.proportion_batches(itemset_sizes)
        bdf = fig.batch2df(batches)
        fig.write_batch_parquet(batchfile, bdf)
    else:
        bdf = fig.read_batch_parquet(batchfile)
        batches = fig.df2batch(bdf)
    print(bdf)
    
    for i in range(numbers):
        start = datetime.now()
        data = fig.generate_from_batch(batches, 300000)
        df = pd.DataFrame({"itemsets": data})
        table = pa.Table.from_pandas(df)
        pq.write_to_dataset(table, filepath)
        end = datetime.now()
        print(i, end-start)


def test_read(filepath: str):
    from pyarrow import parquet as pq
...
if __name__ == "__main__":
    test_generate_parquet("D:/DataGeneration/itemsets_800.parquet", "D:/DataGeneration/batch_800.parquet", 10)
    # test_read("D:/DataGeneration/itemsets.parquet")
    # test_batch("D:/DataGeneration/batch.parquet")